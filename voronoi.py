...
#修改------------------------------------------------------
def bowyer_watson(image, height, width, points, neighbors, output_folder, image_name, extension):
    """ Bowyer-Watson 算法生成 Delaunay 三角剖分和 Voronoi 图 """
    # 创建超级三角形
    sp1 = Point(-math.ceil(width * 1.5), -1)
    sp2 = Point(math.ceil(width * 2.5), -1)
    sp3 = Point(width // 2, math.ceil(height * 2.5))
    super_tri = Triangle(sp1, sp2, sp3)
    ...
    # 移除包含超级三角形顶点的三角形
    delaunay_triangles = [tri for tri in triangulation if not tri.contains_super(super_tri)]
    delaunay = np.zeros((height, width, 3), np.uint8)
    for triangle in delaunay_triangles:
        for edge in triangle:
            p1 = (edge.p1.x, edge.p1.y)
            p2 = (edge.p2.x, edge.p2.y)
    ...
    # 生成 Voronoi 图
    voronoi = voronoi_diagram(delaunay_triangles, neighbors, height, width)
    out = voronoi_painting(voronoi, image, height, width)

    # 保存生成的图像
    cv2.imwrite(os.path.join(output_folder, f"{image_name}-2delaunay.{extension}"), delaunay)
    cv2.imwrite(os.path.join(output_folder, f"{image_name}-3voronoi.{extension}"), voronoi)
    cv2.imwrite(os.path.join(output_folder, f"{image_name}-4out.{extension}"), out)
...
def main():
    if len(sys.argv) != 4:
        print("\n执行方式 (Python 3):")
        print("\tpython voronoi.py image_path N output_folder\n")
        print("image_path -> 图像路径")
        print("N -> 点的数量")
        print("output_folder -> 输出图像的文件夹路径\n")
        exit()
    ...
    extension = extension.lstrip('.')  # 移除点号

    # 高斯模糊去噪
    image = cv2.GaussianBlur(image, (7, 7), 0)

    # 生成随机点（假设您有 points_gen 模块）
    # points = points_gen.random_points(image, num_points)
    # points = points_gen.weighted_random(image, num_points)
    # 这里假设 points 是一个 Point 对象的列表
    import random
    ...
    # 生成显示点的图像
    points_img = np.zeros((height, width, 3), np.uint8)
    for point in points:
        points_img[point.y][point.x] = (0, 255, 0)  # 绿色点

    start_time = time.time()
    neighbors = defaultdict(list)
    out, delaunay, voronoi = bowyer_watson(image, height, width, points, neighbors, output_folder, name, extension)
    print("--- {:.2f} 秒 ---".format(time.time() - start_time))

    # 保存点图像
    cv2.imwrite(os.path.join(output_folder, f"{name}-1points.{extension}"), points_img)

    # 选择是否使用暴力法生成输出图像
    # out = bruteforce(image, height, width, points)

    print("所有图像已保存到文件夹:", output_folder)
